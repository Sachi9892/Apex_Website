...
import { examLinks } from '../data/NavLinks';
...
function ExploreExams() {
    ...
    const getVisibleCountries = () => {
        const countries = [];
        for (let i = 0; i < 3; i++) {
            countries.push(examLinks[(startIndex + i) % examLinks.length]);
        }
        return countries;
    };
    ...
    useEffect(() => {
        const interval = setInterval(() => {
            setStartIndex((prev) => (prev + 1) % examLinks.length);
        }, 2500);
    ...
    const handlePrev = () => {
        setStartIndex((prev) =>
            prev === 0 ? examLinks.length - 1 : prev - 1
        );
    };
    ...
    return (
        <div className="w-full flex flex-col items-center py-10 space-y-8 bg-gray-50">
            <h2 className="text-xl sm:text-2xl md:text-3xl font-bold text-center">
                <strong>Explore Exams With Apex Edutech</strong>
            </h2>
                ...
                <div className="flex flex-col sm:flex-row justify-between w-full">
                    {visibleCountries.map((country, index) => (
                        <div
                            ...
                            className="w-full ml-2 sm:w-1/3 bg-white rounded-lg shadow hover:shadow-md transition p-4 flex flex-col items-center text-center cursor-pointer mb-4 sm:mb-0"
                        >
